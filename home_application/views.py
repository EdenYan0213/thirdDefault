...
from home_application.constants import MAX_ATTEMPTS, JOB_RESULT_ATTEMPTS_INTERVAL, JOB_BK_BIZ_ID, BK_JOB_HOST, \
    SEARCH_FILE_PLAN_ID, WAITING_CODE, WEB_SUCCESS_CODE, SUCCESS_CODE, BACKUP_FILE_PLAN_ID
...
def get_sets_list(request):
    ...
    "fields": ["bk_set_id", "bk_set_name", "bk_biz_id", "bk_created_at",
    "bk_supplier_account"],
    }
...
def search_file(request):
    ...
    host_id_list = [int(bk_host_id) for bk_host_id in
                    host_id_list_str.split(",")]
    kwargs = {
        "bk_scope_type": "biz",
    ...
    while attempts < MAX_ATTEMPTS:
        # 获取执行方案执行状态配置路由在home_application / urls.py文件配置接口路由：
